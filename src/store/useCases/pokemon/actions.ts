...
export const pokemonAsyncActions = {
  getPokemons: createAsyncThunk(
    `${prefix}/getPokemons`,
    async (
      input: WithSuccess<api.GetPokemons['input'], boolean> | undefined,
      thunkApi,
    ) => {
      try {
...
export type FulfilledActionType = Record<
  keyof typeof types,
  ActionCreatorWithPayload<any, string>
>

export const fulfilledActions = {
  getPokemons: createAction<api.GetPokemons['output']>(
    types.getPokemons.fulfilled,
  ),
  getPokemonById: createAction<api.GetPokemonById['output']>(
    types.getPokemonById.fulfilled,
  ),
